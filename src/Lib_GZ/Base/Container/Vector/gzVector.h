...
namespace GzVector{
	...
	template<class T> inline void fTabAssign(T* _aTab, const gzUIntX _nSize,  const gzVecSized<T>&  _vVec){
		Pragma_Unroll_8
	...
	template<class T> inline void fTabAdd(T* _aNew, const T* _aTab, const gzUIntX _nSize,  const T* _aOtherTab, const gzUIntX _nOtherSize){ //SameSize?
...
#define gzDef_Vec_Func(_Name, _nSize) \
	...
	inline gzVec##_Name operator+(const gzVecSized<T>&   _vVec) const {gzVec##_Name _vNew = {0};GzVector::fTabAdd(_vNew.aTab,aTab,_nSize,_vVec.aTab,_vVec.nSize);return _vNew;}\
	...
	inline void  fPrint() const {GzVector::fPrint(gzContTab(aTab), GZ_TabSize );}\
